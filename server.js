/**
 * Multi-user Chat — Theme toggle (Cloud Cat / Fox) + Unread Count + Notifications
 * - 인원 제한 없음
 * - 읽음 표시: "안 읽은 사람 수" 실시간 카운트
 * - 알림: 데스크톱 알림(🔕 토글), 탭 깜빡임, 상단 뱃지, 삑소리/진동
 * - 테마(구름/여우) 토글, 이모지(입력창 삽입), 파일 전송, 이미지 라이트박스
 * - Enter 전송, 자동 스크롤, keep-alive
 * - 메모리형(재시작 시 기록 없음)
 */
...
const APP_VERSION = 'v-2025-09-22-notify';
 ...
 *   key: string|null,
 *   users: Set<socketId>,
 *   lastMsgs: Array<{t:number, from:socketId}>,
 *   unread: Map<msgId, Set<socketId>>
 * }
 */
...
<script>
  ...
  // Focus/visibility for read & notification
  var hasFocus = document.hasFocus();
  var visible = document.visibilityState === 'visible';
  function isAttended(){ return hasFocus && visible; }
  window.addEventListener('focus', function(){ hasFocus = true; clearUnseen(); rescanUnread(); stopFlash(); });
  window.addEventListener('blur', function(){ hasFocus = false; });
  document.addEventListener('visibilitychange', function(){
    visible = document.visibilityState === 'visible';
    if (visible) { clearUnseen(); rescanUnread(); stopFlash(); }
  });
  ...
  function updateNotifyBtn(){
    ...
    notifyBtn.title = notifyEnabled ? '알림 켜짐' : '알림 꺼짐';
  }
  updateNotifyBtn();
  notifyBtn.onclick = function(){
    if (!('Notification' in window)) { alert('브라우저가 알림을 지원하지 않아요.'); return; }
    if (Notification.permission === 'granted') { notifyEnabled = !notifyEnabled; updateNotifyBtn(); }
    else { Notification.requestPermission().then(function(p){ notifyEnabled = (p==='granted'); updateNotifyBtn(); }); }
  };
  ...
  function unlockAudioOnce(){
    ...
    try{
      ...
      o.start(); setTimeout(function(){ try{o.stop();}catch(e){} }, 50);
      audioUnlocked = true;
    }catch(e){}
  }
  ...
  // Unseen counter + title flasher
  var baseTitle = document.title;
  var titleFlasher = null;
  var unseenTotal = 0;
  function startFlash(){
    ...
    titleFlasher = setInterval(function(){
      on = !on;
      document.title = on ? '🔔 ' + unseenTotal + ' 새 메시지' : baseTitle;
    }, 1000);
  }
  function stopFlash(){ if (titleFlasher){ clearInterval(titleFlasher); titleFlasher=null; document.title = baseTitle; } }
  function bumpUnseen(){
    unseenTotal++;
  ...
  function onIncoming(kind, payload){
    if (isAttended()) return; // 창 보고 있으면 알람 불필요
    if (kind === 'msg'){
  ...
  function sendMsg(){
    ...
    clearUnseen(); // 내가 보낸 순간은 보고 있는 상태
  }
